//
//  UIViewController+ZXExtension.m
...
static const void *kNavigationBarHidden = &kNavigationBarHidden;
static const void *kNavigationBarKey = &kNavigationBarKey;

@implementation UIViewController (ZXExtension)

@dynamic navigationBar, scrollingNavigationBarDelegate;
...
+ (void)load
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = @selector(hideNavigationBar_viewWillAppear:);
...
{
    [self setZx_interactiveNavigationBarHidden:self.zx_interactiveNavigationBarHidden animated:animated];
}

- (BOOL)zx_interactiveNavigationBarHidden
{
    return [objc_getAssociatedObject(self, kNavigationBarHidden) boolValue];
}

- (void)setZx_interactiveNavigationBarHidden:(BOOL)zx_interactiveNavigationBarHidden
{
    [self setZx_interactiveNavigationBarHidden:zx_interactiveNavigationBarHidden animated:NO];
}

- (void)setZx_interactiveNavigationBarHidden:(BOOL)hidden animated:(BOOL)animated
{
    objc_setAssociatedObject(self, kNavigationBarHidden, @(hidden), OBJC_ASSOCIATION_COPY);
    [self.navigationController setNavigationBarHidden:hidden animated:animated];
}

- (UINavigationBar *)navigationBar
{
    UINavigationBar *navbar = objc_getAssociatedObject(self, &kNavigationBarKey);
    
    if (!navbar) {
        
        [self zx_loadNavigationBar];
    }
...
{
    objc_setAssociatedObject(self, @selector(useSuperview), [NSNumber numberWithBool:useSuperview], OBJC_ASSOCIATION_RETAIN);
}

- (BOOL)useSuperview
{
    return [objc_getAssociatedObject(self, @selector(useSuperview)) boolValue];
}


- (void)setExpandOnActive:(BOOL)expandOnActive
{
    objc_setAssociatedObject(self, @selector(expandOnActive), [NSNumber numberWithBool:expandOnActive], OBJC_ASSOCIATION_RETAIN);
}

- (BOOL)expandOnActive
{
    return [objc_getAssociatedObject(self, @selector(expandOnActive)) boolValue];
}


- (void)setScrollingNavigationBarDelegate:(id <ScrollingNavigationBarDelegate>)scrollingNavigationBarDelegate
{
    objc_setAssociatedObject(self, @selector(scrollingNavigationBarDelegate), scrollingNavigationBarDelegate, OBJC_ASSOCIATION_ASSIGN);
}

- (id <ScrollingNavigationBarDelegate>)scrollingNavigationBarDelegate
{
    return objc_getAssociatedObject(self, @selector(scrollingNavigationBarDelegate));
}


- (void)setScrollableViewConstraint:(NSLayoutConstraint *)scrollableViewConstraint
{
    objc_setAssociatedObject(self, @selector(scrollableViewConstraint), scrollableViewConstraint, OBJC_ASSOCIATION_RETAIN);
}

- (NSLayoutConstraint *)scrollableViewConstraint
{
    return objc_getAssociatedObject(self, @selector(scrollableViewConstraint));
}


- (void)setScrollableHeaderConstraint:(NSLayoutConstraint *)scrollableHeaderConstraint
{
    objc_setAssociatedObject(self, @selector(scrollableHeaderConstraint), scrollableHeaderConstraint, OBJC_ASSOCIATION_RETAIN);
}

- (NSLayoutConstraint *)scrollableHeaderConstraint
{
    return objc_getAssociatedObject(self, @selector(scrollableHeaderConstraint));
}


- (void)setScrollableHeaderOffset:(float)scrollableHeaderOffset
{
    objc_setAssociatedObject(self, @selector(scrollableHeaderOffset), [NSNumber numberWithFloat:scrollableHeaderOffset], OBJC_ASSOCIATION_RETAIN);
}

- (float)scrollableHeaderOffset
{
    return [objc_getAssociatedObject(self, @selector(scrollableHeaderOffset)) floatValue];
}
...
- (void)setCollapsed:(BOOL)collapsed
{
    ...
    {
        if ([self.scrollingNavigationBarDelegate respondsToSelector:@selector(navigationBarDidChangeToCollapsed:)]) {
            [self.scrollingNavigationBarDelegate navigationBarDidChangeToCollapsed:collapsed];
        }
    }
...
- (void)setExpanded:(BOOL)expanded
{
    ...
    {
        if ([self.scrollingNavigationBarDelegate respondsToSelector:@selector(navigationBarDidChangeToExpanded:)])
        {
            [self.scrollingNavigationBarDelegate navigationBarDidChangeToExpanded:expanded];
        }
    }
...
- (BOOL)expanded
{
    return [objc_getAssociatedObject(self, @selector(expanded)) boolValue];
}
...
{
    objc_setAssociatedObject(self, @selector(maxDelay), [NSNumber numberWithFloat:maxDelay], OBJC_ASSOCIATION_RETAIN);
}

- (float)maxDelay
{
    return [objc_getAssociatedObject(self, @selector(maxDelay)) floatValue];
}


- (void)setDelayDistance:(float)delayDistance
{
    objc_setAssociatedObject(self, @selector(delayDistance), [NSNumber numberWithFloat:delayDistance], OBJC_ASSOCIATION_RETAIN);
}

- (float)delayDistance
{
    return [objc_getAssociatedObject(self, @selector(delayDistance)) floatValue];
}
...
- (void)updateSizingWithDelta:(CGFloat)delta
{
    ...
    CGRect frameNav = self.navigationBar.frame;
...
- (void)updateNavbarAlpha:(CGFloat)delta
{
    ...
    for (UIView *item in self.navigationBar.subviews) {
